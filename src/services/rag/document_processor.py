...
class DocumentProcessor:
    ...
    def _get_loader(self, file_path: str, file_type: str):
        ...
        loaders = {
            ...
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document": lambda: Docx2txtLoader(str(path)),
        ...
        if not loader_factory:
            ...
            if ext in fallback_loaders:
                logger.warning(f"Неизвестный MIME-тип {file_type}, определяю по расширению {ext}")
                return fallback_loaders[ext](str(path))
